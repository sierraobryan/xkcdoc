...
class FavoriteListFragment : BaseFragment() {
    ...
    private fun setUpAdapter(list: List<FavoriteComic>) {
        adapter = FavoriteAdapter(list, { comic : FavoriteComic -> comicItemClicked(comic) })
        favorite_recyclerview.adapter = adapter
        favorite_recyclerview.layoutManager = LinearLayoutManager(activity)
    }

    private fun comicItemClicked(comic : FavoriteComic) {
        switchFragment(SingleComicFragment.newInstance(ComicTag(comic.comicId, comic.title, "")))
    }
}